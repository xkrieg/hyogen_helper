...
def user_by_week(directory):
    ...
    agg_df = df.groupby(['user', pd.Grouper(key='date', freq='W-MON')])['ave_word_len','spell_check','grammar_check'].mean().reset_index().sort_values('date')
    agg_df['word_count'] = df.groupby(['user', pd.Grouper(key='date', freq='W-MON')])['word_count'].sum().reset_index().sort_values('date')['word_count']
    agg_df['longest_word'] = df.groupby(['user', pd.Grouper(key='date', freq='W-MON')])['longest_word'].max().reset_index().sort_values('date')['longest_word']
