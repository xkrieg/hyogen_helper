...
paper_rater['class'] = list_of_essay_files[0].split("/")[1]
paper_rater['assignment'] = list_of_essay_files[0].split("/")[2]
for essay_file in list_of_essay_files[1:]:
    essays = pd.read_csv(essay_file)
    essays['class'] = essay_file.split("/")[1]
    essays['assignment'] = essay_file.split("/")[2]
...
for index, row in paper_rater.iterrows():
    ...
    df.append([row['grade'], row['class'], row['assignment'], row['revision']] + 
              check_essay(essay))
    
#Create masterfile
df = pd.DataFrame.from_records(df)
df.columns=['grade', 'class', 'assignment', 'prepost', 'paragraph_count',
            'sentence_count','word_count', 'num_problem_spaces', 'num_misspelt',
